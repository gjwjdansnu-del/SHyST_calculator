...
// NASA 다항식 계수 (시뮬레이터용 - 간소화)
const NASA_COEFFS = {
    n2: {
        low: [3.298677e0, 1.408240e-3, -3.963222e-6, 5.641515e-9, -2.444855e-12],
        high: [2.926640e0, 1.487977e-3, -5.684761e-7, 1.009704e-10, -6.753351e-15]
    },
    air: {
        low: [3.298677e0, 1.408240e-3, -3.963222e-6, 5.641515e-9, -2.444855e-12],
        high: [2.926640e0, 1.487977e-3, -5.684761e-7, 1.009704e-10, -6.753351e-15]
    ...
    h2: {
        low: [2.344331e0, 7.980521e-3, -1.947815e-5, 2.015721e-8, -7.376117e-12],
        high: [2.932865e0, 8.266079e-4, -1.464023e-7, 1.541003e-11, -6.888048e-16]
    },
    co2: {
        low: [2.356773e0, 8.984596e-3, -7.123562e-6, 2.459190e-9, -1.436995e-13],
        high: [4.636594e0, 2.741319e-3, -9.958285e-7, 1.603730e-10, -9.161034e-15]
    }
};
...
function calcCpOverR_NASA(T, gasType) {
    ...
    if (!coeffs) return 3.5; // 기본값
    
    const a = (T < 1000) ? coeffs.low : coeffs.high;
    const T2 = T * T;
    const T3 = T2 * T;
    const T4 = T3 * T;
    
    return a[0] + a[1]*T + a[2]*T2 + a[3]*T3 + a[4]*T4;
}

// 온도에 따른 gamma 계산 (NASA 다항식)
function calcGammaFromT(T, gasType) {
    const cpOverR = calcCpOverR_NASA(T, gasType);
    return cpOverR / (cpOverR - 1);
}

// 혼합 가스의 온도 의존 gamma (NASA 다항식)
function calcGammaFromT_mix(T, X_He) {
    ...
    return cpOverR_mix / (cpOverR_mix - 1);
}
...
class ShockTube1D {
    ...
    getGammaR(Y_driver, T) {
        ...
        // 온도 의존 gamma 계산
        let gamma_driver, gamma_driven;
        
        if (this.config.driverGas === 'mix') {
            // 혼합 가스
            gamma_driver = calcGammaFromT_mix(T, this.config.heRatio);
        } else {
            gamma_driver = calcGammaFromT(T, this.config.driverGas);
        }
        
        gamma_driven = calcGammaFromT(T, this.config.drivenGas);
        
        // cv 계산
        const cv_driver = dp.R / (gamma_driver - 1);
        const cv_driven = vp.R / (gamma_driven - 1);
        
        const cv_mix = Y_driver * cv_driver + Y_driven * cv_driven;
    ...
    consToPrim() {
        ...
        for (let i = 0; i < nx; i++) {
            ...
            // 초기 gamma로 온도 추정
            const props_init = this.getGammaR(Y[i], 300);
            gamma[i] = props_init.gamma;
            R[i] = props_init.R;
            ...
            // 온도로 gamma 재계산 (1회 반복)
            const props = this.getGammaR(Y[i], T[i]);
            gamma[i] = props.gamma;
            p[i] = Math.max((gamma[i] - 1) * rho[i] * e[i], 1e-10);
            T[i] = p[i] / (rho[i] * R[i]);
